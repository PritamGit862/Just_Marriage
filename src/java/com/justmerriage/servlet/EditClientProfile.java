...
public class EditClientProfile extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            String oldFile = client.getProfile();
            client.setProfile(imageName);
            client.setShopName(shopName);
            client.setAddress(address);
            //Update on Data base
            ClientDao clientDao = new ClientDao(ConnectionProvider.getConnection());
            //out.print("ans");
            boolean ans = clientDao.updateUser(client);

            if (ans) {
                String path = request.getRealPath("/") + "image" + File.separator + client.getProfile();
